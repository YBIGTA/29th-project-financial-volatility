...
중간발표 피드백 중 "이데일리 단일 뉴스 소스의 정치적 편향" 지적이 있었습니다. 실제로 찾아보니 이데일리는 진보 성향으로 분류되는 경제지라, 반대 성향(우파/중도우파) 경제지를 하나 더 넣어서 완화하려고 여러 후보를 확인했습니다.
...
**아직 안 한 것**: 최종적으로 소스가 하나 추가되면, `sentiment_score.py`의 `collect_all_text()`에 반영하고 "뉴스 소스 여러 개 vs 커뮤니티 소스 여러 개 가중치를 어떻게 줄지"도 같이 정리해야 합니다 — 그 파일은 승곤님 담당이라 임의로 안 건드렸습니다(지금은 소스별 건수 그대로 다 섞어서 평균 내다보니 토스 비중이 압도적임).
...
- **뉴스 소스 정치 편향 완화용 대체 소스를 아직 못 찾았습니다** — 매일경제까지 폐기하고 현재 동아일보를 확인 중입니다 (위 참고). 결정되면 `sentiment_score.py`에 소스 추가·소스별 가중치 조정까지 승곤님과 같이 정리해야 완료됩니다.
