...
후보를 세 곳 확인했는데 둘은 접근 자체가 안 됐습니다 — **한국경제**(robots.txt는 허용이지만 Cloudflare 봇 차단, 사람이 직접 스크립트를 돌려도 서버가 봇으로 판별하는 방식이라 그대로 막힘), **조선비즈**(기사 목록을 JS로 나중에 그려주는 방식이라 일반 요청으론 안 보임, 되게 하려면 헤드리스 브라우저가 필요해서 보류). **매일경제**만 남았는데, 여긴 페이지 접근은 되지만 robots.txt가 ClaudeBot을 이름으로 명시해서 막아뒀습니다(`User-agent: ClaudeBot / Disallow: /`, `User-agent: *`는 허용). 그래서 이 스크립트도 네이버 통합검색과 같은 이유로 자동 파이프라인에 넣지 않고 직접 실행해야 합니다.

종목명으로 검색 시 브라우저가 실제로 부르는 내부 API를 지훈님이 개발자도구로 직접 확인해줘서 알아냈습니다 (robots.txt 때문에 제가 직접 사이트를 열어서 확인은 못 함):

```
GET https://www.mk.co.kr/_CP/243?word={검색어}&page={N}&highlight=Y&page_size=null&id=null
```

응답은 HTML 조각이고, 기사 하나당 카테고리/제목/본문요약/날짜 링크 4개가 전부 같은 기사 URL을 가리키며 반복되는 구조입니다. 정렬이 기본 최신순이라, 팍스넷처럼 페이지를 넘기다 설정 기간보다 오래된 기사가 나오면 멈추는 방식으로 만들었습니다 — 이데일리·팍스넷과 같은 급으로 기간 전체를 확보할 수 있습니다 (RSS 방식은 최신 몇십 건뿐이라 폐기).
...
**신뢰도 주의**: robots.txt 때문에 제가 실제 페이지를 열어서 확인할 수 없어서, 지훈님이 화면에서 복사해준 텍스트만 보고 HTML 구조를 추정해서 만들었습니다. 실행했는데 계속 0건만 나오면 `BlockedOrChangedPageError` 메시지부터 확인하고, 필요하면 실제 응답 HTML 일부를 다시 공유해주세요.
