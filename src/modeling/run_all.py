"""
전체 파이프라인 한 번에 실행 (v9)
로컬(VS Code 등)에서 `python run_all.py`로 01~05번을 순서대로 실행.
각 단계는 개별적으로도 실행 가능 (예: 코드만 확인하고 싶을 때 python 02_garch_models.py 등).

실행 순서: 01(피처 빌드) -> 02(GARCH) -> 03(XGBoost, 튜닝 포함) -> 05(통합 유의성 검정)
Target은 pk(Parkinson, 메인) / 5d(rolling_5d, 부록) 둘 다 자동으로 산출됨.
"""
import subprocess
import sys
